...
export default function AccountModal({ user, onClose, onUpdate, onSignOut }) {
  ...
  return <div className="modal-backdrop" onClick={onClose} role="presentation"><section className="account-modal" onClick={(event) => event.stopPropagation()}>
    <button className="close" type="button" onClick={onClose} aria-label="Close">&times;</button><p className="eyebrow">YOUR HARUKA ACCOUNT</p><h2>Account</h2><div className="account-email">{user.email}</div><div className="role-badge">{user.role === "admin" ? "Admin" : "Member"}</div>
